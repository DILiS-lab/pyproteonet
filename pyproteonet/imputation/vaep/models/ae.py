...
from ..io.datasets import DatasetWithTarget
from ..io.dataloaders import get_test_dl
from ..transform import VaepPipeline
from ..io.dataloaders import get_dls
from ..models import calc_net_weight_count
...
def get_preds_from_df(df: pd.DataFrame,
                      ...
                      dataset: torch.utils.data.Dataset = DatasetWithTarget):
...
class AutoEncoderAnalysis(analysis.ModelAnalysis):
    ...
    def __init__(self,
        ...
        self.transform =  VaepPipeline(
                                      df_train=train_df,
                                      encode=transform,
                                      decode=decode)
        self.dls = get_dls(train_X=train_df,valid_X=val_df,
        ...
        self.n_params_ae = calc_net_weight_count(self.model)
    ...
    def get_test_dl(self, df_wide:pd.DataFrame, bs:int=64) -> pd.DataFrame:
        return get_test_dl(df=df_wide, transformer=self.transform, bs=bs)
