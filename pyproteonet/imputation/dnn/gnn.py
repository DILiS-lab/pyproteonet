...
class ImputationModuleBackup(L.LightningModule):
    def __init__(self, molecule, partner_molecule, mapping, in_dim, layers, lr=0.1, dropout = 0.2, gat_heads=10, mask_value=-10):
        super().__init__()
        ...
        self.molecule_gat = GATv2Conv(in_feats=(partner_fc_out_dim, in_dim), out_feats=2*in_dim, num_heads=gat_heads, feat_drop=dropout, attn_drop=dropout)
...
class ImputationModule(L.LightningModule):
    def __init__(self, molecule, partner_molecule, mapping, in_dim, layers,
                 molecule_loss_coefficent=1, partner_loss_coefficent=1,
                 lr=0.1, dropout = 0.2, gat_heads=10, gat_dim=64, mask_value=-10):
        super().__init__()
        self.molecule = molecule
        self.partner_molecule = partner_molecule
        self.mapping = mapping
        ...
        self.partner_fc_model = nn.Sequential(*dense_layers)
        partner_fc_out_dim = 2 * in_dim
        self.molecule_gat = GATv2Conv(in_feats=(partner_fc_out_dim, in_dim), out_feats=gat_dim, num_heads=gat_heads,
                                      feat_drop=dropout, attn_drop=dropout)
        self.partner_gat = GATv2Conv(in_feats=(gat_dim, partner_fc_out_dim), out_feats=gat_dim, num_heads=gat_heads,
                                     feat_drop=dropout, attn_drop=dropout, allow_zero_in_degree=True)
        self.molecule_gat2 = GATv2Conv(in_feats=(gat_dim, gat_dim), out_feats=gat_dim, num_heads=gat_heads,
                                       feat_drop=dropout, attn_drop=dropout)
    ...
    def forward(self, graph):
        ...
        mol_vec = nn.functional.leaky_relu(self.molecule_gat(graph, (partner_vec, molecule_inputs)).mean(dim=-2))
        partner_vec = nn.functional.leaky_relu(self.partner_gat(inverse_graph, (mol_vec, partner_vec)).mean(dim=-2))
        mol_vec = nn.functional.leaky_relu(self.molecule_gat2(graph, (partner_vec, mol_vec)).mean(dim=-2))
        #output = output.view(output.shape[0], -1)
    ...
    def compute_loss(self, graph)->torch.tensor:
        ...
        #loss = torch.nn.functional.mse_loss(output, inputs.mean(dim=-1, keepdim=True))
        loss = (self.molecule_loss_coefficent * self.loss_fn(mol_pred, target=mol_target, var=mol_var) +
                self.partner_loss_coefficent * self.loss_fn(partner_pred, target=partner_target, var=partner_var))
        return loss
    ...
    def predict_step(self, graph, batch_idx, dataloader_idx=0):
        mol_vec, partner_vec  = self(graph)
        return mol_vec[:,:,0], torch.exp(mol_vec[:,:,1])
...
def impute_gnn(dataset: Dataset, molecule: str, column: str, mapping: str, partner_column: str,
               result_column: Optional[str] = None, uncertainty_column: Optional[str] = None,
               molecule_coefficient = 1, partner_coefficient = 1,
               max_epochs: int = 5000)->pd.Series:
    ...
    missing_mols = missing_mols[missing_mols.isna()].index.get_level_values('id').unique()
    mapped = dataset.get_mapped(molecule=molecule, partner_molecule=partner_molecule, mapping=mapping, partner_columns=[partner_column])
    ...
    mapped_non_missing = mapped[~mapped.index.get_level_values(molecule).isin(missing_mols)]
    non_missing_fraction = (mapped_non_missing.isna().sum() / mapped_non_missing.shape[0]).item()
    ...
    ds.rename_columns(columns={molecule:{column:'abundance'}, partner_molecule:{partner_column:'abundance'}}, inplace=True)
    ...
    partner_validation_ids = partner_validation_ids[~partner_validation_ids.isna()].sample(frac=0.1).index
    validation_set = MaskedDataset.from_ids(dataset=ds, mask_ids={molecule:validation_ids, partner_molecule: partner_validation_ids})
    def masking_fn(in_ds):
        ...
        molecule_mask_ids = molecule_mask_ids[~molecule_mask_ids.index.isin(validation_ids)].sample(frac=0.1).index
        partner_mask_ids = in_ds.values[partner_molecule]['abundance']
        partner_mask_ids = partner_mask_ids[~partner_mask_ids.isna()]
        partner_mask_ids = partner_mask_ids[~partner_mask_ids.index.isin(partner_validation_ids)].sample(frac=masking_fraction).index
        return MaskedDataset.from_ids(dataset=in_ds, mask_ids={molecule:molecule_mask_ids, partner_molecule:partner_mask_ids},
    ...
    def collate(mds):
        res = []
        for md in mds:
            graph = md.to_dgl_graph(molecule_features={molecule:'abundance', partner_molecule:'abundance'}, mappings=[mapping],
    ...
    model = ImputationModule(molecule=molecule, partner_molecule=partner_molecule, mapping=mapping, in_dim=in_dim,
                             layers=[8*in_dim, 8*in_dim,4*in_dim], gat_heads=2*in_dim, gat_dim=4*in_dim,
                             molecule_loss_coefficent=molecule_coefficient, partner_loss_coefficent=partner_coefficient,
                             dropout=0.1, lr=0.01)

    trainer = L.Trainer(logger=ConsoleLogger(), log_every_n_steps = 10, check_val_every_n_epoch=10, max_epochs=max_epochs, enable_checkpointing=False,
                        callbacks=[EarlyStopping(monitor="val_loss", mode="min", patience=3)])
    trainer.fit(model=model, train_dataloaders=train_dl, val_dataloaders=validation_dl)

    missing_mds = mask_missing(dataset=ds, molecule=molecule, column='abundance')
    pred_dl = DataLoader([missing_mds], batch_size=1, collate_fn=collate)
    pred, uncertainty = trainer.predict(model=model, dataloaders=pred_dl, ckpt_path=None)[0]
    pred, uncertainty = pred.numpy(), uncertainty.numpy()

    missing_mds.set_samples_value_matrix(matrix=pred, molecule=molecule, column='abundance', only_set_masked=True)
    normalizer.unnormalize(dataset=ds, inplace=True)

    missing_mds.set_samples_value_matrix(matrix=uncertainty * normalizer.stds[molecule]['abundance'], molecule=molecule, column='uncertainty', only_set_masked=True)

    res = dataset.values[molecule][column]
    mask = res[res.isna()].index
    res.loc[mask] = ds.values[molecule]['abundance'].loc[mask]
    if result_column is not None:
        dataset.values[molecule][result_column] = res
    if uncertainty_column is not None:
        uncertainty = ds.values[molecule]['uncertainty']
        uncertainty[~uncertainty.index.isin(mask)] = np.nan
        dataset.values[molecule][uncertainty_column] = uncertainty
    return res