...
class ImputationModule(L.LightningModule):
    def __init__(
        ...
        self.molecule_embedding_map = nn.Sequential(nn.Linear(
            embedding_in_dim, molecule_embedding_shrink_dim
        ), nn.Dropout(p=dropout))
        self.partner_embedding_map = nn.Sequential(nn.Linear(embedding_in_dim, embedding_shrink_dim), nn.Dropout(p=dropout))
        if self.use_sample_embeddings:
            sample_emb_dim = 32
            self.samples_embeddings = nn.Embedding(num_samples, sample_emb_dim)
        else:
            sample_emb_dim = 0
        for dense_layers, fc_in_dim in [
            (dense_layers_partner, in_dim + embedding_shrink_dim + sample_emb_dim)
        ]:  # , (dense_layers_molecule, embedding_shrink_dim + sample_emb_dim)]:
            last_dim = fc_in_dim
            for dim in layers:
        ...
        self.molecule_gat = HeteroGraphConv(
            {
                self.etype: GATv2Conv(
                    in_feats=(
                        fc_out_dim,
                        molecule_embedding_shrink_dim + sample_emb_dim,
                    ),
                    out_feats=gat_dim,
                    num_heads=gat_heads,
                    feat_drop=dropout,
        ...
        self.molecule_gat2 = HeteroGraphConv(
            {
                self.etype: GATv2Conv(
                    in_feats=(gat_dim + fc_out_dim, molecule_embedding_shrink_dim),
                    out_feats=gat_dim,
                    num_heads=gat_heads,
                    feat_drop=dropout,
                    attn_drop=dropout,
    ...
    def forward(self, batch, abundance_column: str = "abundance"):
        ...
        if self.use_sample_embeddings:
            sample_emb_partner = sample_emb.repeat(partner_inputs[0].shape[0], 1)
            partner_inputs.append(sample_emb_partner)
        partner_inputs = torch.cat(partner_inputs, dim=-1)
        ...
        molecule_fc_vec = molecule_inputs  # self.molecule_fc_model(molecule_inputs)
        mol_vec = nn.functional.leaky_relu(
            self.molecule_gat(
                graph,
                (
                    {self.partner_molecule: partner_fc_vec},
                    {self.molecule: molecule_fc_vec},
                ),
        ...
        partner_vec = nn.functional.leaky_relu(
            self.partner_gat(
                graph,
                ({self.molecule: mol_vec}, {self.partner_molecule: partner_fc_vec}),
        ...
        mol_vec = nn.functional.leaky_relu(
            self.molecule_gat2(
                graph, ({self.partner_molecule: partner_vec}, {self.molecule: mol_emb})
            )[self.molecule].mean(dim=-2)
        )
...
def impute_sequ_embedding_gnn(
    ...
    non_missing_per_missing_mol = (
        ...
        .groupby([molecule, "sample"])
        .abundance.count()
    )
    non_missing_per_missing_mol.index.set_names("id", level=molecule, inplace=True)
    ...
    non_missing_per_missing_mol = non_missing_per_missing_mol[
        non_missing_per_missing_mol.index.isin(missing_mols)
    ...
    def find_mask_ids(in_ds: Dataset):
        ...
        partner_ids = partner_mask_ids.sample(  # [~partner_mask_ids.index.isin(partner_validation_ids)]
        ...
        candidates = mapped.groupby(["sample", molecule]).abundance.count()
        candidates = candidates[
            candidates.index.set_names("id", level=molecule).isin(non_missing_mols)
        ]
        candidates = candidates.map(non_missing_per_missing_mol)
        candidates[candidates.isna()] = 0
        molecule_ids = candidates.sample(
            frac=2 * epoch_masking_fraction, weights=candidates
        ).index
        #molecule_ids = non_missing_mols
    ...
    if train_sample_wise:
        ...
        layer_dim_dict = {
            ...
            "gat_heads": 4 * num_samples,
            "gat_dim": int(num_samples / 2),
        }
