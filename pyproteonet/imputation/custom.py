...
def multistep_impute(dataset: Dataset, molecule: str, column: str, mapping: str, partner_column: str, all_missing_percentile: float = 0.1)->pd.Series:
    top3 = partner_top_n_mean(dataset=dataset, molecule=molecule, partner_column=partner_column, mapping=mapping, top_n=3, only_unique=True, 
                               skip_if_less_than_n=False)
    median = partner_aggregation(dataset=dataset, molecule=molecule, partner_column=partner_column, mapping=mapping, only_unique=False, method='median')
