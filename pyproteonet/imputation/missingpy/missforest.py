    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    def _miss_forest(self, Ximp, mask):
        ...
        if self.num_vars_ is not None:
            ...
            Ximp[missing_num_rows, missing_num_cols] = np.take(col_means, missing_num_cols)

            # Reg criterion
            reg_criterion = self.criterion if type(self.criterion) == str else self.criterion[0]
        ...
        if self.cat_vars_ is not None:
            ...
            clf_criterion = self.criterion if type(self.criterion) == str else self.criterion[1]
        ...
        while (gamma_new < gamma_old or gamma_newcat < gamma_oldcat) and self.iter_count_ < self.max_iter:
            ...
            if self.cat_vars_ is not None:
                gamma_newcat = np.sum((Ximp[:, self.cat_vars_] != Ximp_old[:, self.cat_vars_])) / n_catmissing
            if self.num_vars_ is not None:
                gamma_new = np.sum((Ximp[:, self.num_vars_] - Ximp_old[:, self.num_vars_]) ** 2) / np.sum(
                    (Ximp[:, self.num_vars_]) ** 2
                )
    ...
    def fit(self, X, y=None, cat_vars=None):
        ...
        force_all_finite = False if (self.missing_values == "NaN" or np.isnan(self.missing_values)) else True
        ...
        if cat_vars is not None:
            if type(cat_vars) == int:
                ...
                if np.array(cat_vars).dtype != int:
                    raise ValueError("cat_vars needs to be either an int or an array " "of ints.")
            else:
                raise ValueError("cat_vars needs to be either an int or an array " "of ints.")
    ...
    def transform(self, X):
        ...
        if n_cols_X != n_cols_fit_X:
            raise ValueError("Incompatible dimension between the fitted " "dataset and the one to be transformed.")
