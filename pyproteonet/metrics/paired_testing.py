...
def paired_wilcoxon_matrix(dataset: Dataset, molecule:str, eval_columns: List[str], gt_column: str,
                           comparison_metric: Union[Literal['AE', 'SE'], Callable[[pd.Series, pd.Series], pd.Series]] = 'AE',
                           ids: Optional[pd.Index] = None, ax: Optional[matplotlib.axes.Axes] = None, significance_level: float = 0.05):
    ...
    if ax is not None:
        ax.matshow(res_mat, cmap=matplotlib.colors.LinearSegmentedColormap.from_list("", ["green","grey","red"]))
