...
def map_uniprot(ids: List[str], to_db: str, from_db: str = 'UniProtKB_AC'):
    job_id = submit_id_mapping(from_db=from_db, to_db=to_db, ids=list(ids))
    print(job_id)
...
#     Args:
#         input (pd.Series): Pandas Series to map.
#         map_field (str): Field that is mapped to.
...
#     """
#     uniprot_ids = input
#     base_url = "http://rest.uniprot.org/uniprotkb/idmapping"
#     start = 0
#     results = []
...
#             'to': map_field,
#             #"format": "tsv",
#             "ids": ",".join([id for id in uniprot_ids[start : start + request_size]]),
...
#         df_result.columns = ["entry", map_field]
...
#     #results.loc[results[map_field].isna(), map_field] = ""
#     return results